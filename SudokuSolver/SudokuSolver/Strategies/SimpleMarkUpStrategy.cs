...
namespace SudokuSolver.Strategies
{
    class SimpleMarkUpStrategy : ISudokuStrategy
    {
            ...
            for (int row = 0; row < sudokuBoard.GetLength(0); row++)
            {
                for (int col = 0; col < sudokuBoard.GetLength(1); col++)
                {
                    if(sudokuBoard[row,col] == 0 || sudokuBoard[row,col].ToString().Length > 1)
                    {
                        ...
                        sudokuBoard[row, col] = GetPossiblityIntersection(possibilitiesInRowAndCol, possibilitiesInBlock);
                    }
                }
            }
            return sudokuBoard;
        ...
        private int GetPossiblityIntersection(object possibilitiesInRowAndCol, object possibilitiesInBlock)
        {
